...
public class AuthController {
    ...
    public ApiResponse<AuthResponse.SignUpResponseDTO> signup(@RequestBody AuthRequest.SignUpDTO request) {
        return ApiResponse.onSuccess(null);
    }

    @PostMapping("/login")
    @Operation(summary = "로그인 API", description = "이메일과 비밀번호를 이용하여 로그인하고 액세스 토큰을 발급받는 API입니다.")
    public ApiResponse<AuthResponse.LoginResponseDTO> login(@RequestBody AuthRequest.LoginDTO request) {
        return ApiResponse.onSuccess(null);
    }
}
