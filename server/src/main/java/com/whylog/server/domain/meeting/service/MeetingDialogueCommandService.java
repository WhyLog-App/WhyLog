...
public class MeetingDialogueCommandService {
    ...
    /**
     * 발화 한 건을 저장합니다. 발화마다 호출되므로 연관 엔티티는 프록시로만 잡아 INSERT 한 번으로 끝냅니다. meetingId·memberId는 웹소켓 핸드셰이크에서
     * 검증된 값이라 존재 여부를 다시 조회하지 않습니다.
     */
    public void appendSpeech(
            Long meetingId, Long memberId, String text, LocalDateTime speechDateTime) {
        Meeting meeting = meetingRepository.getReferenceById(meetingId);
        Member member = memberRepository.getReferenceById(memberId);
        dialogueRepository.save(Dialogue.create(meeting, member, text, speechDateTime));
    }
}
