...
public class MeetingRoomRepository {
    ...
    // 특정 멤버의 모든 연결 세션을 회의방에서 제거합니다.
    public List<MeetingParticipant> removeParticipantsByMemberId(Long memberId) {
        Set<String> sessionIds = sessionIdsByMemberId.remove(memberId);
        if (sessionIds == null || sessionIds.isEmpty()) {
            return List.of();
        }

        List<MeetingParticipant> removedParticipants = new ArrayList<>();
        for (String sessionId : new ArrayList<>(sessionIds)) {
            MeetingParticipant removed = participantsBySessionId.remove(sessionId);
            if (removed != null) {
                removedParticipants.add(removed);
            }
        }
