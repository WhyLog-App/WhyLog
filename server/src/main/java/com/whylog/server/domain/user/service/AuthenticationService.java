...
public class AuthenticationService {
    ...
    public AuthResponse.LoginResponseDTO generateLoginResponse(Member member) {
        ...
        UsernamePasswordAuthenticationToken authentication = createAuthentication(member.getId(), role, authorities);
    ...
    public AccessTokenGenerateResponse generateAccessTokenFromRefreshToken(String refreshToken) {
        ...
        if (validationType != JwtValidationType.VALID_JWT) {
            throw new ErrorHandler(switch (validationType) {
                case EXPIRED_JWT_TOKEN -> AuthErrorStatus.REFRESH_TOKEN_EXPIRED;
                case INVALID_JWT_TOKEN, INVALID_JWT_SIGNATURE, UNSUPPORTED_JWT_TOKEN, EMPTY_JWT ->
                        AuthErrorStatus.INVALID_REFRESH_TOKEN;
                default -> AuthErrorStatus.INVALID_REFRESH_TOKEN;
            });
        }
        ...
        if (!memberId.equals(storedMemberId)) {
            throw new ErrorHandler(AuthErrorStatus.INVALID_REFRESH_TOKEN);
        }

        Role role = jwtTokenProvider.getRoleFromJwt(refreshToken);
        Collection<GrantedAuthority> authorities = List.of(role.toGrantedAuthority());
        UsernamePasswordAuthenticationToken authentication = createAuthentication(memberId, role, authorities);
