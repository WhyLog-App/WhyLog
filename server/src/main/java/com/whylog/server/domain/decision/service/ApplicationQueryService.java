...
public class ApplicationQueryService {
    ...
    public ApplicationResponse.ApplicationDetailDTO getApplicationDetail(Long applicationId) {
        ...
        Map<Long, MemberDisplayResolver.DisplayMember> displayMembersById =
                memberDisplayResolver.resolveByTeamMemberIds(
                        application.getDecision().getMeeting().getTeam().getId(),
                        memberIds(applicationTimelines));
        ...
        return ApplicationResponse.ApplicationDetailDTO.builder()
                ...
                .decisionContexts(toDecisionContextItems(applicationTimelines, displayMembersById))
                ...
                .map(
                        applicationTimeline -> {
                            ...
                            MemberDisplayResolver.DisplayMember displayMember =
                                    memberId == null ? null : displayMembersById.get(memberId);
                            if (displayMember == null) {
                                displayMember = memberDisplayResolver.resolve(null, null);
                            }
    ...
    // 타임라인에 포함된 발화자 ID를 중복 없이 구성한다.
    private List<Long> memberIds(List<ApplicationTimeline> applicationTimelines) {
        return applicationTimelines.stream()
