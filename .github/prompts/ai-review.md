...
- `docs/pr-reviews/`의 자동 생성 기록은 코드 리뷰 대상이나 규칙 근거로 사용하지 않는다.
...
`file`은 diff에 나온 저장소 상대 경로를 그대로 사용한다.
`line`은 변경 후 파일의 절대 줄 번호이며 GitHub diff의 오른쪽(RIGHT)에 실제로 보이는 줄만 사용한다.
정확한 오른쪽 줄을 특정할 수 없으면 추측하지 말고 `null`을 사용한다.
